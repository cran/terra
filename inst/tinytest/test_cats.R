...
expect_equal(names(r_cat), c("val1", "val2"))
expect_false(any(is.factor(r_cat)))
expect_silent(show(r_cat))
expect_equal(as.vector(values(r_cat[[1]])), as.vector(values(r)) + 100)
expect_equal(as.vector(values(r_cat[[2]])), as.vector(values(r)) * 0.1)

r_num <- as.numeric(r)
expect_false(any(is.factor(r_num)))
expect_equal(names(r_num), "val1")
expect_equivalent(as.vector(values(r_num)), as.vector(values(r)))
